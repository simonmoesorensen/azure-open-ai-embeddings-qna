...
def get_pgrace_event():
    from pgrace.concar.event import Event

    # Check env is set
    if not os.getenv("GRACE_USER_TOKEN"):
        raise Exception(
            "Please set the environment variable GRACE_USER_TOKEN with your token"
        )

    if not os.getenv("GRACE_URL"):
        raise Exception("Please set the environment variable GRACE_URL with your url")

    event = Event("Azure-AI")
    event.SetGovStream("Solution-Accelerator")

    event.SetCustomStr1("Chat")
    event.SetCustomStr2("Test-Id")
    event.SetComment("Chat for Azure's Solution Acclerator project")
    return event
...
def check_deployment():
    ...
    try:
        ...
        st.error(
            f"""Embedding model is not working.
    ...
    try:
        ...
        st.error(
            f"""Translation model is not working.
    ...
    try:
        ...
        if llm_helper.vector_store_type != "AzureSearch":
            ...
            try:
                ...
                st.error(
                    """Azure Cognitive Search is not working.
...
try:
    ...
    if st.session_state.askedquestion != "":
        ...
        (
            ...
            st.session_state["context"],
            st.session_state["sources"],
        ) = llm_helper.get_semantic_answer_lang_chain(st.session_state["question"], [])
        (
            st.session_state["response"],
        ...
        # Set and send governance event
        event = get_pgrace_event()
        event.SetPredictionInput(st.session_state.askedquestion)
        latency = time.time() - time_start
        event.SetLatency(latency)
        event.SetPredictionResult(
            st.session_state["response"]
            + " \nFollow up questions: "
            + str(st.session_state["followup_questions"])
        )
        event.SendAndClear()
    ...
    if (
        ...
        and st.session_state["translation_language"] != ""
    ):
